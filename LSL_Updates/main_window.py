...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QListWidget, QStackedWidget,
...
from subject_id_handshake import HandshakeState, HandshakeResult
from experiment_controller import CmdResult, SequencerState
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # ── Subject ID ─────────────────────────────────────────────────────────
        self.gb_subject = QGroupBox("Subject ID")
        sl = QHBoxLayout(self.gb_subject)
        self.subject_input = QSpinBox()
        self.subject_input.setRange(1, 9999)
        self.subject_input.setValue(1)
        self.subject_confirm_btn = QPushButton("Confirm")
        self.subject_status_lbl = QLabel("Not sent")
        sl.addWidget(QLabel("Subject:"))
        sl.addWidget(self.subject_input)
        sl.addWidget(self.subject_confirm_btn)
        sl.addStretch(1)
        sl.addWidget(self.subject_status_lbl)
        bl.addWidget(self.gb_subject)

        # ── Experiment Controls ────────────────────────────────────────────────
        self.gb_ctrl = QGroupBox("Experiment Controls")
        cv = QVBoxLayout(self.gb_ctrl)
        btn_row = QHBoxLayout()
        self.btn_step       = QPushButton("▶ Step")
        self.btn_force_step = QPushButton("⚡ Force Step")
        self.btn_session_1  = QPushButton("Session 1")
        self.btn_session_2  = QPushButton("Session 2")
        self.btn_session_3  = QPushButton("Session 3")
        for b in (self.btn_step, self.btn_force_step,
                  self.btn_session_1, self.btn_session_2, self.btn_session_3):
            btn_row.addWidget(b)
            b.setEnabled(False)   # gated until SUBJECT_ID_ACK
        cv.addLayout(btn_row)

        # ── State Display ──────────────────────────────────────────────────────
        self.lbl_state = QLabel("Sequencer dormant — confirm subject ID first.")
        self.lbl_state.setStyleSheet("font-family: monospace; padding: 6px;")
        cv.addWidget(self.lbl_state)
        bl.addWidget(self.gb_ctrl)
    ...
    def _wire(self):
        ...
        # ── Subject-ID + experiment-control wiring ─────────────────────────────
        self.subject_confirm_btn.clicked.connect(self._on_confirm_subject)
        self.btn_step.clicked.connect(lambda: self._unity.experiment_controller.send_step())
        self.btn_force_step.clicked.connect(lambda: self._unity.experiment_controller.send_force_step())
        self.btn_session_1.clicked.connect(lambda: self._unity.experiment_controller.send_session(1))
        self.btn_session_2.clicked.connect(lambda: self._unity.experiment_controller.send_session(2))
        self.btn_session_3.clicked.connect(lambda: self._unity.experiment_controller.send_session(3))

        self._unity.subject_id_acked.connect(self._on_subject_id_acked)
        self._unity.subject_id_rejected.connect(self._on_subject_id_rejected)
        self._unity.subject_id_state.connect(self._on_subject_id_state)
        self._unity.unity_ready.connect(self._on_unity_ready)
        self._unity.exp_state.connect(self._on_exp_state)
        self._unity.cmd_done.connect(self._on_cmd_done)
    ...
    def _start_rec(self):
        ...
        # Hard gate: refuse to start without a confirmed subject ID. The
        # button is disabled in _update_start_btn for this case but defense
        # in depth — a keyboard shortcut or a stale state during reconnect
        # could otherwise let recording start without a subject. Without a
        # subject ID, the Unity side won't create its session folder either,
        # so the recordings would be one-sided and impossible to align.
        if self._unity.subject_id_handshake.confirmed_subject_id is None:
            self._log("[REC] Start refused — Subject ID not confirmed.")
            QMessageBox.warning(
                self, "Subject ID required",
                "Confirm the Subject ID before starting a recording.\n\n"
                "The Unity side will not create a session folder without one, "
                "so a recording started here would have no Unity counterpart "
                "to align against.",
            )
            return
    ...
    def _update_start_btn(self):
        ...
        # Subject-ID gate runs first — nothing else matters without it. The
        # Unity side refuses to create a session folder without a confirmed
        # subject ID, so starting LSL recording without one would produce a
        # one-sided recording with no Unity counterpart to align against.
        subject_confirmed = (
            self._unity.subject_id_handshake.confirmed_subject_id is not None
        )

        if not subject_confirmed:
            self._btn_start.setEnabled(False)
            self._btn_start.setToolTip("Confirm Subject ID first")
        elif not required:
            self._btn_start.setEnabled(False)
    ...
    # ── Subject-ID + experiment-control slots ──────────────────────────────────

    def _on_confirm_subject(self):
        sid = int(self.subject_input.value())
        ok = self._unity.subject_id_handshake.send_subject_id(sid)
        if not ok:
            self.subject_status_lbl.setText(f"Invalid: {sid}")

    def _on_subject_id_state(self, state):
        # state is a HandshakeState enum
        self.subject_status_lbl.setText({
            "idle":     "Not sent",
            "sending":  "Sending…",
            "acked":    "Confirmed ✓",
            "rejected": "Rejected ✗",
            "timeout":  "Timed out — Unity not reachable",
        }.get(state.value, state.value))

    def _on_subject_id_acked(self, sid):
        self.subject_input.setEnabled(False)
        self.subject_confirm_btn.setEnabled(False)
        for b in (self.btn_step, self.btn_force_step,
                  self.btn_session_1, self.btn_session_2, self.btn_session_3):
            b.setEnabled(True)
        # Refresh the Start Recording gate — subject ID was the missing
        # condition; with it confirmed, the button may now enable depending
        # on device readiness.
        self._update_start_btn()

    def _on_subject_id_rejected(self, result):
        # result is HandshakeResult
        if result.reason == "already_set" and result.existing_id is not None:
            reply = QMessageBox.question(
                self, "Subject ID conflict",
                f"Unity already locked Subject {result.existing_id:02d}. "
                f"Override with {result.subject_id}?",
            )
            if reply == QMessageBox.StandardButton.Yes:
                self._unity.subject_id_handshake.send_subject_id(
                    result.subject_id, allow_override=True)
            else:
                self.subject_input.setValue(result.existing_id)
        else:
            QMessageBox.warning(self, "Subject ID rejected",
                                f"Reason: {result.reason}")

    def _on_unity_ready(self, info):
        sid = info.get("subject_id")
        if sid is not None:
            self.subject_input.setValue(sid)
            self.subject_input.setEnabled(False)
            self.subject_confirm_btn.setEnabled(False)
            for b in (self.btn_step, self.btn_force_step,
                      self.btn_session_1, self.btn_session_2, self.btn_session_3):
                b.setEnabled(True)
            self.subject_status_lbl.setText(f"Re-locked ✓ (subject {sid:02d})")
            # Round-trip an idempotent SUBJECT_ID so the local handshake state
            # transitions to ACKED — otherwise confirmed_subject_id stays None
            # and the Start Recording gate would falsely block. Unity's
            # TrySetSubjectID is idempotent for an unchanged id, so this is
            # safe and just causes a re-ACK.
            self._unity.subject_id_handshake.send_subject_id(sid)
        else:
            self.subject_status_lbl.setText("Unity ready — awaiting subject ID")
        self._update_start_btn()

    def _on_exp_state(self, s):
        self.lbl_state.setText(
            f"  Session {s.session_index}  ({s.session_label})\n"
            f"  Trial   {s.trial}/{s.total_trials}\n"
            f"  Violation: {s.violation}\n"
            f"  GSM: {s.gsm_state}"
        )

    def _on_cmd_done(self, cmd):
        # Optional flash of cmd outcome — left as no-op so the file change
        # is minimal. Can be wired to a status bar later.
        pass
